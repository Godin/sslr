...
// TODO should contribute all children to parent
public class FirstOfExpression extends AbstractCompilableMatcher implements CompilableMatcher {

  private final CompilableMatcher[] subExpressions;

  public FirstOfExpression(CompilableMatcher... subExpressions) {
  ...
  public Instr[] compile() {
    ...
    for (int i = 0; i < subExpressions.length - 1; i++) {
      ...
      result.add(null);
      // add program
      addAll(result, subExpressions[i].compile());
      // add placeholder for "Commit"
      result.add(null);
      offsets[i] = result.size();
    }
    // add last program
    addAll(result, subExpressions[subExpressions.length - 1].compile());
